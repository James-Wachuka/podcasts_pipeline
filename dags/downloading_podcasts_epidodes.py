...
@dag(dag_id='downloading_episodes',default_args={'owner': 'airflow'}, schedule_interval=timedelta(days=1), start_date=pendulum.datetime(2022, 1, 1, tz=local_tz), catchup=False)
def marketplace_podcast():
    ...
    def download_episodes(episodes):
        for episode in episodes:
            filename = episode['link'].split('/')[-1]
            filepath = os.path.join('mnt/c/data_eng/podcasts_pipeline/episodes', filename)
            if not os.path.exists(filepath):
                response = requests.get(episode['enclosure']['@url'])
    ...
    download_episodes_task = download_episodes(download_task)
    download_task >> create_table >> load_task >> download_episodes_task
